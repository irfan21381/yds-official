...
export default function Sidebar() {
  const linkClass = ({ isActive }: { isActive: boolean }) =>
    `block px-4 py-3 rounded-md font-medium ${
      isActive ? "bg-blue-600 text-white" : "text-gray-700 dark:text-gray-200 hover:bg-gray-100"
    }`;

  const externalLinkClass = "block px-4 py-3 rounded-md font-medium text-gray-700 dark:text-gray-200 hover:bg-gray-100";

  return (
    <aside className="fixed left-0 top-0 h-full w-60 bg-white dark:bg-gray-900 border-r dark:border-gray-700">
      <div className="p-4 border-b dark:border-gray-800">
        <h1 className="text-xl font-bold text-blue-600">🌟YDS🌟</h1>
      </div>
      <nav className="p-4 space-y-2">
        ...
        {/* AI Assistant Link - Using <a> to avoid 404 */}
        <a href="https://yasindigitalsolutions.online/" className={externalLinkClass} target="_blank" rel="noopener noreferrer">
          AI Assistant
        </a>
      </nav>
    </aside>
